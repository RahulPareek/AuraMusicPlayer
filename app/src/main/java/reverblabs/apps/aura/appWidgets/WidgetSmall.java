...
public class WidgetSmall extends AppWidgetProvider {
    ...
    public static void checkEnabled(Context context, AppWidgetManager manager){
        widgetEnabled = manager.getAppWidgetIds(new ComponentName(context, WidgetSmall.class)).length != 0;
    }
...
//        }
    ...
    public static void updateWidget(Song song, Context context, AppWidgetManager appWidgetManager, MediaMetadataCompat metadataCompat){
        ...
        if(widgetEnabled) {
            ...
            remoteViews.setViewVisibility(R.id.widget_small_song_name, View.VISIBLE);
            remoteViews.setViewVisibility(R.id.widget_small_artist_name, View.VISIBLE);

            remoteViews.setTextViewText(R.id.widget_small_song_name, song.getTitle());
            remoteViews.setTextViewText(R.id.widget_small_artist_name, song.getArtist());

            if (metadataCompat != null) {
                remoteViews.setImageViewBitmap(R.id.widget_small_album_art, metadataCompat.getBitmap(MediaMetadataCompat.METADATA_KEY_ALBUM_ART));
            }
            else {
                remoteViews.setImageViewResource(R.id.widget_small_album_art, R.drawable.album_art);
            }
...
//            }
            ...
            remoteViews.setOnClickPendingIntent(R.id.widget_small_album_art, pendingIntent);

            appWidgetManager.updateAppWidget(new ComponentName(context, WidgetSmall.class), remoteViews);
